...
import 'package:erp_mobile/features/projects/domain/entities/task.dart';
import 'package:erp_mobile/features/projects/domain/usecases/move_task.dart';
...
void main() {
  group('moveTask', () {
    test('no-op when target equals current status', () {
      ...
      expect(moveTask(task: t, toStatus: TaskStatus.inProgress), same(t));
    });
    ...
    test('any active → done is allowed', () {
      for (final s in [
        ...
        expect(
          moveTask(task: _t(status: s), toStatus: TaskStatus.done).status,
          TaskStatus.done,
        );
      }
    });
