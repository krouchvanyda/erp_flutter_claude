...
class CallkitEventHandler {
  ...
  /// `VoiceCallScreen.isMounted` / `VideoCallScreen.isMounted` checks in
  ...
  Future<void> _handleAccept(dynamic body) async {
    ...
    // `VoiceCallScreen.isMounted` / `VideoCallScreen.isMounted`).
  ...
  Future<void> _pushCallPageWithRetry({
    ...
    final alreadyMounted = isVideo
        ? VideoCallScreen.isMounted
        : VoiceCallScreen.isMounted;
    if (alreadyMounted) {
      // ignore: avoid_print
    ...
    for (var attempt = 0; attempt < 25; attempt++) {
      ...
      if (navigator != null) {
        ...
        print('[CallkitEventHandler] step 2 → pushing '
            '${isVideo ? "VideoCallScreen" : "VoiceCallScreen"}'
