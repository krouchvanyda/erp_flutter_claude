...
class _VoiceCallPageState extends State<VoiceCallPage>
  ...
  /// allowed"). Flip the button to muted + warn so the UI matches reality, and
  /// so the page's later mic re-applies keep it off. Idempotent; runs on connect.
  Future<void> _syncMicWithPermission() async {
    if (_muted) return;
    final granted = await isMicrophoneGranted();
    if (granted || !mounted || _muted) return;
