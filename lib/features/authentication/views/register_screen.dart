...
import 'package:erp_mobile/features/authentication/view_models/register_view_model.dart';

class RegisterScreen extends StatefulWidget {
  const RegisterScreen({super.key, this.onSimulatedRegister});
...
class _RegisterPageState extends State<RegisterScreen> {
  ...
  /// Validate the form + terms, then hand off to the ViewModel.
  /// Success/failure side-effects are handled in [_onRegisterState].
  ...
  void _onRegisterState(BuildContext context, RegisterState state) {
    switch (state.status) {
      case RegisterStatus.success:
        widget.onSimulatedRegister?.call();
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => RegisterViewModel(AppDependencies.I.authRepository),
      child: BlocListener<RegisterViewModel, RegisterState>(
        listenWhen: (previous, current) => previous.status != current.status,
        listener: _onRegisterState,
                                  ...
                                  children: [
                                    ...
                                    BlocBuilder<RegisterViewModel, RegisterState>(
                                      ...
                                      builder: (context, state) => SizedBox(
                                        height: 54,
                                        child: FilledButton(
                                          // Disables while the request is
                                          // in flight — one visual signal
