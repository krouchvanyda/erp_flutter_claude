...
void main() {
  ...
  late RejectInvoiceUseCase usecase;
  final fixedClock = DateTime.utc(2026, 5, 13, 11, 30);

  setUp(() {
    repo = _MockRepo();
    perms = _MockPerms();
    usecase = RejectInvoiceUseCase(
      repository: repo,
      permissions: perms,
      clock: () => fixedClock,
    );
  ...
  group('RejectInvoiceUseCase', () {
    test('blank reason → ValidationFailure (BEFORE permission/state checks)',
      ...
      await expectLater(
        usecase(invoiceId: 'inv-1', approverId: 'u', reason: '  '),
    ...
    test('forbidden when caller lacks finance.approve', () async {
      ...
      await expectLater(
        usecase(invoiceId: 'inv-1', approverId: 'u', reason: 'no'),
    ...
    test('notFound when id is unknown', () async {
      ...
      await expectLater(
        usecase(invoiceId: 'gone', approverId: 'u', reason: 'no'),
