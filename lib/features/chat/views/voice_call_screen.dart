...
class VoiceCallScreen extends StatefulWidget {
  const VoiceCallScreen({super.key, required this.conversationId});

  final String conversationId;

  /// Tracks whether a VoiceCallScreen is currently mounted, so the
  ...
  @override
  State<VoiceCallScreen> createState() => _VoiceCallPageState();
}

enum _CallStage { calling, ringing, connected, ended }

class _VoiceCallPageState extends State<VoiceCallScreen>
  ...
  void initState() {
    super.initState();
    VoiceCallScreen.isMounted = true;
    ...
    // ignore: avoid_print
    print('[VoiceCallScreen] 🟢 MOUNTED · conversationId=${widget.conversationId} '
  ...
  @override
  void dispose() {
    VoiceCallScreen.isMounted = false;
    // ignore: avoid_print
    print('[VoiceCallScreen] 🔴 DISPOSED · conversationId=${widget.conversationId} '
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // ignore: avoid_print
    print('[VoiceCallScreen] lifecycle → $state · active=${_signaling.current?.callId}'
    ...
    if (state == AppLifecycleState.resumed) {
      // ignore: avoid_print
      print('[VoiceCallScreen] resumed → calling signaling.reconcileActive() '
  ...
  void _onActiveCallChanged() {
    ...
    if (call == null) {
      // ignore: avoid_print
      print('[VoiceCallScreen] _onActiveCallChanged · call=NULL — '
      ...
      Future.delayed(const Duration(milliseconds: 200), () {
        if (mounted && Navigator.canPop(context)) {
          // ignore: avoid_print
          print('[VoiceCallScreen] popping route now (after 200 ms grace)');
    ...
    // ignore: avoid_print
    print('[VoiceCallScreen] _onActiveCallChanged · ${call.callId}/${call.state} '
